...
module type S = sig
  ...
  val sign : Private_key.t -> Message.t -> network_id:int -> Signature.t
...
module Make
  ...
  let sign (d_prime : Private_key.t) ~(network_id : int) m =
    let public_key =
...
module Message = struct
  ...
  let derive t ~private_key ~public_key ~(network_id : int) =
    let input =
